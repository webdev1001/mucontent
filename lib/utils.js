...
function restricted (req, res, next) {
    if (req.session.user) { 
        ...
        // Get the route
        var count = 0, size = parameters.routing_acl.length;
        // Iterate the acl array
        parameters.routing_acl.forEach(function (item) {     
...
function sendmail (to, subject, template, locales, language) {
    if (parameters.allow_mail) { // Check if the email is allowed
        ...
        var transport = nodemailer.createTransport("SES", {
            AWSAccessKeyID: parameters.AWS_KEY,
            AWSSecretKey: parameters.AWS_SECRET
        });
        ...
        var source = fs.readFileSync(__dirname + "/../template/" + language + "/" + template + ".html", 'utf8'); 
        var compiled = handlebars.compile(source);
        var mail = compiled(locales);
        // For subject, get locales and create json, then find by handle
        var locales = fs.readFileSync(__dirname + "/../locales/" + language + ".json", 'utf8'); 
